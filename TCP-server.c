...
#define PORT "10011"  // the port users will be connecting to
...
int main()
{
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			int readResult = read(new_fd, buffer, MAXBUFLEN);
			if (readResult < 0)
			{
				perror("read");
			}
			...
			int numOperands = *(uint8_t *) (buffer + 3);
			int operand1 = *(uint16_t *) (buffer + 4);
